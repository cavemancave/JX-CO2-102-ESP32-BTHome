...
## CO2 Beacon Extension

This project is extended to read CO2 ppm values from a UART sensor and broadcast them via BLE advertising once per second.

- UART: `UART1`, RX=GPIO4, TX=GPIO5, 9600 8N1
- Expected line format from sensor: e.g. `400 ppm\r\n`
- Range clamped to 0–5000 ppm
- BLE Advertising payload: Manufacturer Specific Data with layout
    - Company ID: 0x02E5 (Espressif)
    - Type: 0x01 (CO2)
    - Value: 16-bit little-endian CO2 ppm
- Scan Response: Complete device name

You can inspect the advertising data in nRF Connect → Manufacturer Data (length 5 bytes). The CO2 value updates as new UART lines arrive.
